...
(function (root, factory) {
  ...
  // two DIFFERENT watched-cell sets deliberately give two different
  ...
  //     — a temporary +-1 wobble around wherever the rotation anchor
  //     currently is.
  //   - CYCLE: a strict superset, the top two rows plus the three fastest
  //     cells of the third row (cells 0-10)
  //       no strokes at all (every watched cell REST), OR
  //       every watched cell shows BOTH strokes
  //     — either condition advances the rotation anchor ONE STEP through
  //     THEMES (wrapping), discarding accumulated drift. (An earlier version
  //     distinguished these two conditions as jumps to two FIXED targets,
  //     "black" and "white" — removed: the anchor is meant to visit the
  //     whole palette over time, not alternate between two poles, so both
  //     conditions now do the same single thing.)
  ...
  // drift direction an empirically verified average recurrence of ~1.71 days:
  ...
  // cycle bar is also a valid drift-cell state, just one the drift
  // classifier's stricter all-A/all-B test never matches) makes a
  // cycle-advance meaningfully rarer — ~54.9 days on average — dozens of
  // drift steps between cycle-advances, while staying on a
  // human-observable timescale rather than centuries. A cell is 2 bits of
  // Gray-code state, but each output bit i of Gray code depends on input
  // bits i AND i+1 (g = n ^ (n>>1)), so N watched cells (2N output bits)
  // actually depend on 2N+1 input bits — the watched pattern's true period
  // is 2^(2N+1) bars, not 2^(2N): each extra cycle-watch cell QUADRUPLES the
  // cycle period, not doubles it. Because the cycle cells' own repeating
  // period is a whole multiple of the drift cells' period, one combined
  // table (driven by the larger cycle-cell set) covers both: the whole
  // sequence of trigger events repeats exactly every 2^23 = 8388608 bars
  // (~219.42 days) forever, so instead of scanning for matches at runtime,
  // the full per-cycle event list is precomputed once and embedded below.
  // Within one PATTERN_CYCLE_LEN, drift always nets to exactly zero, but
  // cycle-advances do NOT (there's no matching "retreat" for a cycle
  // advance) — themeIndexForBar in clock-state.js therefore has to count
  // total cycle-advances across however many full cycles have elapsed since
  // bar 0, not just within the current one, for the rotation anchor to
  // genuinely walk forward through all of THEMES over time rather than
  // resetting every ~219 days. Regenerate with
  ...
  var PATTERN_ANCHOR_INDEX = THEMES.findIndex(function (t) { return t.token === PATTERN_ANCHOR_TOKEN; });
  ...
  return {
    ...
    PATTERN_ANCHOR_TOKEN: PATTERN_ANCHOR_TOKEN,
    PATTERN_ANCHOR_INDEX: PATTERN_ANCHOR_INDEX,
    THEMES: THEMES,
    ENVELOPE: ENVELOPE,
