...
### Live landing mark (planned SVG reconstruction)

Eventually reconstruct the mark as a responsive SVG system:

- Row 1: stable, readable, never transforms.
- Row 2: the active transformation layer.
- Row 3: vertical reflection of Row 2 (derived via SVG reference/transform —
  no independent state).
- Row 4: vertical reflection of Row 1 (also stable, also derived).
- Each letter is an independently transformable SVG group.
- Permitted transforms on Row 2 letters only: horizontal reflection, vertical
  reflection, 180° rotation, combined H+V reflection, occasional position
  swaps, restoration to canonical.
- Slow, irregular intervals; frequently returns to the fully readable state.
  No constant motion, jitter, flicker, glitch, arbitrary-angle rotation, or
  elastic distortion.
- `prefers-reduced-motion`: static canonical state.
- Optional interaction: click a letter → permitted transform; click the active
  row → one constrained random mutation; pointer over the mark may pause the
  autonomous sequence.
